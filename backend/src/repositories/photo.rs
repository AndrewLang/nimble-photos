    ...
    async fn get_timeline(&self, limit: u32, offset: u32) -> DataResult<Vec<TimelineGroup>> {
        ...
        let sql = r#"
            SELECT 
                ...
                COALESCE(p_agg.total_count, 0) as total_count,
                COALESCE(p_agg.photos_json, '[]'::json) as photos_json
            FROM (
                SELECT DISTINCT DATE(COALESCE(date_taken, created_at)) as day_date
                FROM photos
            ...
            LEFT JOIN LATERAL (
                SELECT 
                    ...
                    json_agg(pd.* ORDER BY sort_date DESC) as photos_json
                FROM (
                    SELECT 
                    ...
                    WHERE DATE(COALESCE(p.date_taken, p.created_at)) IS NOT DISTINCT FROM td.day_date
                ) pd
            ) p_agg ON true
