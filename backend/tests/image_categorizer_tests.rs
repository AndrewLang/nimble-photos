...
use nimble_photos::services::image_categorizer::{CategorizeRequest, ImageCategorizer, TemplateCategorizer};
use std::fs;
...
fn template_categorizer_supports_hash_shortcut() {
    ...
    properties.insert::<PathBuf>(working_dir.clone()).alias(WORKING_DIRECTORY);
    properties.insert::<String>("abcdef1234567890".to_string()).alias(HASH);
    properties.insert::<Option<chrono::DateTime<Utc>>>(None).alias(EXIF_DATE_TAKEN);
...
fn template_categorizer_supports_date_shortcut_and_custom_template() {
    ...
    date_properties.insert::<PathBuf>(working_dir.clone()).alias(WORKING_DIRECTORY);
    date_properties.insert::<Option<chrono::DateTime<Utc>>>(Some(date_taken)).alias(EXIF_DATE_TAKEN);
    ...
    assert_eq!(date_result.final_path, working_dir.join("2024-01-02").join("incoming-date.jpg"));

    let mut custom_properties = PropertyMap::new();
    custom_properties.insert::<PathBuf>(working_dir.clone()).alias(WORKING_DIRECTORY);
    custom_properties.insert::<Option<chrono::DateTime<Utc>>>(Some(date_taken)).alias(EXIF_DATE_TAKEN);
    custom_properties.insert::<String>("abcdef1234567890".to_string()).alias(HASH);

    let custom_result = TemplateCategorizer::new("{year}/{hash:0:2}/{fileStem}-{hash:0:6}.{extension}")
        .categorize(&CategorizeRequest::new(&source_custom, &custom_properties))
        .expect("custom template should pass");
    assert_eq!(custom_result.final_path, working_dir.join("2024").join("ab").join("incoming-custom-abcdef.jpg"));
}
...
fn template_categorizer_requires_working_directory() {
    ...
    let mut properties = PropertyMap::new();
    properties.insert::<Option<chrono::DateTime<Utc>>>(Some(Utc::now())).alias(EXIF_DATE_TAKEN);

    let result =
        TemplateCategorizer::new("{year}/{fileName}").categorize(&CategorizeRequest::new(&source, &properties));
